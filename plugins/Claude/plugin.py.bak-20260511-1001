...
def parse_addressed(text: str, nick: str):
    """If text is a question addressed to the bot by nickname (at start or
    end), return the cleaned question. Otherwise return None.

    Recognized shapes (case-insensitive):
        nick, <q>?    nick: <q>?    nick <q>?
        <q>, nick?    <q> nick?
    """
    text = text.strip()
    if not text:
    ...
    def doPrivmsg(self, irc, msg):
        ...
        if addressed:
            ...
            if self._is_known_command(irc, first):
                return
            question = stripped if stripped.endswith("?") else None
        else:
            question = parse_addressed(text, irc.nick)
